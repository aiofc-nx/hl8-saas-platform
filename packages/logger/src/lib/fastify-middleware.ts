 ...
 * @example
 * ```typescript
 * const options: FastifyLoggerMiddlewareOptions = {
 ...
 * ```
 */
export interface FastifyLoggerMiddlewareOptions {
  /** 是否启用请求日志 */
 ...
 * @example
 * ```typescript
 * const middleware = new FastifyLoggerMiddleware({
 *   enableRequestLogging: true,
 *   enableResponseLogging: true,
 *   excludePaths: ['/health', '/metrics']
 * });
 ...
 * ```
 */
export class FastifyLoggerMiddleware {
  /** 日志记录器实例 */
  private readonly logger: FastifyLogger;
  /** 中间件选项 */
  private readonly options: FastifyLoggerMiddlewareOptions;
   ...
   * ```
   */
  constructor(options: FastifyLoggerMiddlewareOptions = {}) {
    this.options = {
      enableRequestLogging: true,
      enableResponseLogging: true,
      excludePaths: [],
   ...
   * @example
   * ```typescript
   * const middleware = new FastifyLoggerMiddleware(options);
   * app.register(middleware.plugin);
   * ```
   */
  get plugin() {
   ...
   * ```
   */
  updateOptions(options: Partial<FastifyLoggerMiddlewareOptions>): void {
 ...
 * @param options - 中间件选项
 * @returns {FastifyLoggerMiddleware} 日志中间件实例
 *
 * @example
 * ```typescript
 * const middleware = createFastifyLoggerMiddleware({
 *   enableRequestLogging: true,
 *   enableResponseLogging: true,
 *   excludePaths: ['/health', '/metrics']
 * });
 * ```
 */
export function createFastifyLoggerMiddleware(
  options: FastifyLoggerMiddlewareOptions = {}
): FastifyLoggerMiddleware {
  return new FastifyLoggerMiddleware(options);
}
 ...
 * ```
 */
export async function registerFastifyLogger(
  fastify: FastifyInstance,
  options: FastifyLoggerMiddlewareOptions = {}
): Promise<void> {
  const middleware = createFastifyLoggerMiddleware(options);
