...
export function InjectLogger(context?: string): PropertyDecorator {
  return function (target: any, propertyKey: string | symbol) {
    ...
    Object.defineProperty(target, propertyKey, {
      get() {
        // 创建日志记录器实例
        const logger = new FastifyLogger({ level: 'trace' });
...
export function RequestContext(contextKey: keyof RequestContext): ParameterDecorator {
  return function (target: any, propertyKey: string | symbol | undefined, parameterIndex: number) {
    ...
    const existingMetadata = Reflect.getMetadata('requestContext', target, propertyKey) || [];
    existingMetadata[parameterIndex] = contextKey;
    Reflect.defineMetadata('requestContext', existingMetadata, target, propertyKey);
  };
}
...
export function LogLevel(level: 'trace' | 'debug' | 'info' | 'warn' | 'error' | 'fatal'): ClassDecorator & MethodDecorator {
  return function (target: any, propertyKey?: string | symbol, descriptor?: PropertyDescriptor) {
    if (propertyKey && descriptor) {
      ...
      // 类装饰器
      Reflect.defineMetadata('logLevel', level, target);
    }
  };
}
...
export function LogContext(context: Record<string, unknown>): ClassDecorator & MethodDecorator {
  return function (target: any, propertyKey?: string | symbol, descriptor?: PropertyDescriptor) {
    if (propertyKey && descriptor) {
      ...
      // 类装饰器
      Reflect.defineMetadata('logContext', context, target);
    }
  };
}
 ...
 * ```
 */
export function getLogger(context?: string): FastifyLogger {
  const logger = new FastifyLogger();
