 ...
 * ```
 */
export class FastifyLogger implements LoggerInterface {
  /** Pino 日志器实例 */
  ...
  child(bindings: Record<string, unknown>): FastifyLogger {
    ...
    const childLogger = new FastifyLogger(this.config);
    // 替换内部的 Pino 实例
