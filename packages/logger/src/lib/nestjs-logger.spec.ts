...
describe('NestJSLogger', () => {
  ...
  let fastifyLogger: FastifyLogger;
  let consoleSpy: jest.SpyInstance;

  beforeEach(() => {
    // 创建 NestJSLogger 实例
    nestjsLogger = new NestJSLogger({ level: 'trace' });
    fastifyLogger = nestjsLogger.getFastifyLogger();
  ...
  describe('内部 FastifyLogger 访问', () => {
    it('应该提供获取内部 FastifyLogger 的方法', () => {
      const internalLogger = nestjsLogger.getFastifyLogger();
      expect(internalLogger).toBeDefined();
      expect(internalLogger).toBeInstanceOf(FastifyLogger);
    });
  });
});
