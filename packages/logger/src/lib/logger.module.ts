...
import { FastifyLoggerMiddleware } from './fastify-middleware';
  ...
  static forRoot(params: LoggerModuleParams = {}): DynamicModule {
    const providers: any[] = [
      ...
      },
      {
        ...
        useFactory: (moduleParams: LoggerModuleParams) => {
          return new FastifyLogger(moduleParams.config);
        },
    ...
    if (params.enableRequestLogging || params.enableResponseLogging) {
      providers.push({
        ...
        useFactory: (moduleParams: LoggerModuleParams): FastifyLoggerMiddleware => {
          return new FastifyLoggerMiddleware({
 ...
 * ```
 */
export function createLoggerProviders(config: any = {}) {
  return [
    {
      provide: LOGGER_MODULE_PARAMS,
      useValue: config,
 ...
 * ```
 */
export function createLoggerMiddlewareProviders(config: any = {}) {
  return [
    {
      provide: 'FASTIFY_LOGGER_MIDDLEWARE',
      useFactory: (moduleParams: LoggerModuleParams) => {
 ...
 * ```
 */
export function getLoggerInstance(moduleRef: any): FastifyLogger {
 ...
 * ```
 */
export function getLoggerMiddlewareInstance(moduleRef: any): FastifyLoggerMiddleware {
