...
import { LoggerModule, FastifyLogger, InjectLogger, LogMethod, LogPerformance, LogError } from './index';
...
export class UserService {
  ...
  @LogPerformance({ threshold: 500, level: 'info' })
...
export class FastifyAppConfig {
  ...
  static async configureApp(app: FastifyInstance): Promise<void> {
    // 注册日志中间件
    await app.register(require('./fastify-middleware').registerFastifyLogger, {
      enableRequestLogging: true,
      enableResponseLogging: true,
      excludePaths: ['/health', '/metrics', '/favicon.ico'],
      requestIdGenerator: (req: any) => req.headers['x-request-id'] as string || `req-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
    });
    ...
    app.get('/users/:id', async (request, reply) => {
      ...
      const user = await userService.getUser((request.params as any).id);
      return user;
    });
